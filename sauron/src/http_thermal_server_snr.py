...
from snr_logger import sample_snr
...
# Wireless SNR monitoring
WIFI_IFACE = os.environ.get("WIFI_IFACE") or os.environ.get("WIRELESS_IFACE") or "wlan0"


class SNRMonitor:
    def __init__(self, iface: str, min_interval: float = 2.0):
        self.iface = iface
        self.min_interval = min_interval
        self.last_sample: Optional[Dict[str, Union[float, str]]] = None
        self.last_ts = 0.0
        self.lock = threading.Lock()

    def get_sample(self) -> Optional[Dict[str, Union[float, str]]]:
        now = time.time()
        with self.lock:
            if self.last_sample and (now - self.last_ts) < self.min_interval:
                return self.last_sample

        # Avoid holding the lock while calling external commands
        sample = sample_snr(self.iface)
        if sample:
            sample = {**sample, "ts": datetime.utcnow().isoformat() + "Z"}
...
snr_monitor = SNRMonitor(WIFI_IFACE)


def _safe_round(val: Optional[Union[int, float]], digits: int = 2) -> Optional[float]:
    try:
...
<body>
  <header style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
    ...
    <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap; margin-left:auto;">
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
        ...
        <span id="snr-stats" style="font-size:11px;color:var(--muted);padding:4px 8px;border-radius:6px;border:1px solid transparent;background:rgba(255,255,255,0.03);" title="Wireless signal-to-noise ratio"></span>
      </div>
  ...
  const snrStats = document.getElementById("snr-stats");
  ...
  function applyAutoFpsUI(isOn, fpsValue) {
    ...
    async function updateNetworkStats() {
      try {
        ...
        // Update SNR badge
        const snrValue = Number(data.snr_db);
        const signal = Number(data.signal_dbm);
        const noise = Number(data.noise_dbm);
        if (Number.isFinite(snrValue)) {
          const parts = [`SNR: ${snrValue.toFixed(1)} dB`];
          if (Number.isFinite(signal) && Number.isFinite(noise)) {
            parts.push(`(${signal.toFixed(0)} / ${noise.toFixed(0)} dBm)`);
          }
          snrStats.textContent = parts.join(" ");
          const snrColor = snrValue >= 25 ? '#4ade80'
                          : snrValue >= 15 ? '#60a5fa'
                          : snrValue >= 8  ? '#fbbf24'
                          : '#f87171';
          snrStats.style.color = snrColor;
          snrStats.style.borderColor = snrColor;
          snrStats.style.borderWidth = '1px';
          snrStats.style.borderStyle = 'solid';
        } else {
          snrStats.textContent = 'SNR: --';
          snrStats.style.color = 'var(--muted)';
          snrStats.style.borderColor = 'rgba(255,255,255,0.1)';
        }
        ...
        if (snrStats) {
          snrStats.textContent = 'SNR: --';
          snrStats.style.color = '#ef4444';
          snrStats.style.borderColor = '#ef4444';
        }
      }
    }
...
def api_network_stats():
    ...
    snr_sample = snr_monitor.get_sample()
    return jsonify({
        ...
        "current_fps": current_fps,
        "snr_db": _safe_round(snr_sample.get("snr_db")) if snr_sample else None,
        "signal_dbm": _safe_round(snr_sample.get("signal_dbm")) if snr_sample else None,
        "noise_dbm": _safe_round(snr_sample.get("noise_dbm")) if snr_sample else None,
        "snr_source": snr_sample.get("source") if snr_sample else None,
        "snr_ts": snr_sample.get("ts") if snr_sample else None,
    })


@app.route("/api/snr", methods=["GET"])
def api_snr():
    """Get wireless SNR for the configured interface."""
    sample = snr_monitor.get_sample()
    if not sample:
        return jsonify({
            "error": f"Unable to read SNR from interface '{WIFI_IFACE}'. "
                     f"Set WIFI_IFACE env var if your interface is different."
        }), 503

    return jsonify({
        "iface": WIFI_IFACE,
        "snr_db": _safe_round(sample.get("snr_db")),
        "signal_dbm": _safe_round(sample.get("signal_dbm")),
        "noise_dbm": _safe_round(sample.get("noise_dbm")),
